...
#define UART_FULL_THRESH_DEFAULT        (240)
...
static void uart_event_task(void *pvParameters)
{
    ...
    uint8_t* dsub = (uint8_t*) malloc(RD_BUF_SIZE);
        ...
        if(xQueueReceive(uart0_queue, (void * )&event, timeout / portTICK_RATE_MS)) {
            ...
            switch(event.type) {
                ...
                case UART_DATA:
                    ...
                    sub = uart_read_bytes(EX_UART_NUM, dsub, event.size, timeout / portTICK_RATE_MS);
                    // ESP_LOGI(TAG, "read len: %d", sub);
                    // ESP_LOGI(TAG, "dtmp: %s", (char*)dtmp);
                    // ESP_LOGI(TAG, "dtmp[%d]: %d", event.size - 1, dtmp[event.size - 1]);
                    // ESP_LOGI(TAG, "dtmp[%d]: %d", event.size - 2, dtmp[event.size - 2]);
                    // 检测指令结束符(回车换行符结尾)
                    if (event.size > 0) {
                        ...
                        if (finish) {
                            // ESP_LOGI(TAG, "write: ");
                            finish = false;
                            uart_write_bytes(EX_UART_NUM, (const char*) dtmp, len);
                            bzero(dtmp, RD_BUF_SIZE);
                            len = 0;
                        }
                    }         
